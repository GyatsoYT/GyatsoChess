...
# Main history getter
template getMainHistory*(history: HistoryTables, color: Color, move: Move): int =
  ...
  block:
    if prevMove == Move(0):
      ...
      if prevPiece == NoPiece or currPiece == NoPiece:
        ...
        history.counterHistory[prevIdx][prevMove.toSquare][currIdx][currMove.toSquare]
  ...
  block:
    if grandparentMove == Move(0):
      ...
      if gpPiece == NoPiece or currPiece == NoPiece:
        ...
        let currIdx = getPieceIndex(currPiece)
        history.followUpHistory[gpIdx][grandparentMove.toSquare][currIdx][currMove.toSquare]

# Tactical history getter
template getTacticalHistory*(history: HistoryTables, move: Move, board: Board): int =
  block:
...
template getContinuationHistory*(history: HistoryTables, prevMove: Move, grandparentMove: Move, resultMove: Move, board: Board): int =
...
template updateHistoryStat*(stat: var int16, bonus: int) =
  var s = stat.int
  s += (32 * bonus) - (s * abs(bonus)) div 512
  ...
  if bestMove.isCapture or bestMove.isPromotion:
    ...
    if prevMove != Move(0):
      ...
      if prevPiece != NoPiece and currPiece != NoPiece:
        ...
        let currIdx = getPieceIndex(currPiece)
        updateHistoryStat(
          history.counterHistory[prevIdx][prevMove.toSquare][currIdx][bestMove.toSquare],
          bonus
        )
    ...
    if grandparentMove != Move(0):
      ...
      if gpPiece != NoPiece and currPiece != NoPiece:
        ...
        let currIdx = getPieceIndex(currPiece)
        updateHistoryStat(
          history.followUpHistory[gpIdx][grandparentMove.toSquare][currIdx][bestMove.toSquare],
          bonus
        )
    ...
    if move.isCapture or move.isPromotion:
      ...
      if prevMove != Move(0):
        ...
        if prevPiece != NoPiece and currPiece != NoPiece:
          ...
          let currIdx = getPieceIndex(currPiece)
          updateHistoryStat(
            history.counterHistory[prevIdx][prevMove.toSquare][currIdx][move.toSquare],
            penalty
          )
      ...
      if grandparentMove != Move(0):
        ...
        if gpPiece != NoPiece and currPiece != NoPiece:
          ...
          let currIdx = getPieceIndex(currPiece)
          updateHistoryStat(
            history.followUpHistory[gpIdx][grandparentMove.toSquare][currIdx][move.toSquare],
            penalty
          )
