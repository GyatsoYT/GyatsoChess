...
const
  ...
  # Piece-Relative Constants
  KnightNearOurRookBonus = 10
  BishopNearEnemyKingBonus = 15
  RookOnSameFileBonus = 10
  QueenNearEnemyKingBonus = 5
...
proc evaluatePieceRelativePositions(board: Board): int =
  ...
  # --- White Perspective (Attacking Black) ---
  if board.pieceBB[BlackKing] != 0:
    ...
    while bb != 0:
      let sq = popBit(bb)
      let f = fileOf(sq)
      if (FileMasks[f] and (board.pieceBB[WhiteQueen] or (board.pieceBB[
          WhiteRook] and not (1'u64 shl sq)))) != 0:
  ...
  # --- Black Perspective (Attacking White) ---
  if board.pieceBB[WhiteKing] != 0:
    ...
    while bb != 0:
      let sq = popBit(bb)
      let f = fileOf(sq)
      if (FileMasks[f] and (board.pieceBB[BlackQueen] or (board.pieceBB[
          BlackRook] and not (1'u64 shl sq)))) != 0:
...
proc evaluatePawnStructure(board: Board, mgWhite, egWhite, mgBlack,
    egBlack: var int) =
  # White Pawns
...
proc evaluate*(board: Board): int =
  ...
  let whiteNonPawnMat = (board.occupiedBB[White] xor board.pieceBB[
      WhiteKing] xor board.pieceBB[WhitePawn]) != 0
  let blackNonPawnMat = (board.occupiedBB[Black] xor board.pieceBB[
      BlackKing] xor board.pieceBB[BlackPawn]) != 0

  let whiteWraps = (board.occupiedBB[White] xor board.pieceBB[WhiteKing]) ==
      0 # White has ONLY King
  let blackWraps = (board.occupiedBB[Black] xor board.pieceBB[BlackKing]) ==
      0 # Black has ONLY King
