...
# Move* = distinct uint32 is already defined in types.nim
# source: [0..6], target: [7..13], moved: [14..16], captured: [17..19], promoted: [20..22],
# castled: 23, capturedEnPassant: 24, enPassantTarget: [25..31]

# func `==`*(a, b: Move): bool is already defined in types.nim

template create*(move: var Move, source, target, enPassantTarget: Square, moved, captured, promoted: Piece, castled, capturedEnPassant: bool) =
  move = cast[Move](0u32)
...
func `$`*(move: Move): string =
  ...
  if move.promoted != noPiece:
    # Add promotion piece as a single lowercase letter (q, r, b, n)
    case move.promoted
    of queen: result &= "q"
    of rook: result &= "r"
    of bishop: result &= "b"
